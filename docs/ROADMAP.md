...
**Status:** accepted  
**Date:** 2026-08-25  
**Relates-to:** [PDR](PDR.md), [SDD](SDD.md), [SPEC-wallet-api](SPEC-wallet-api.md), [DIAGRAMS](DIAGRAMS.md), [USE-CASE-MAP](USE-CASE-MAP.md)
...
| **P1 Templates** | File-based registry, JSON Schema + Ajv, field flags `wallet` / `public`. Generic schema (title, fields, image, links). Not Animal types. | Invalid `data` fails validation without DynamoDB. |
...
| **P4 Public page** | `GET /p/{publicId}` HTML from **public** fields only, using [`public-page/`](../public-page/) + [SPEC-public-page](SPEC-public-page.md). Always unauthenticated. DynamoDB read (no Redis yet). | Scanner/browser shows the payload fields in theme slots (`GENERIC`: hero + facts + links). |
...
**P1** — file-based template registry, JSON Schema + Ajv, `GENERIC` v1.
