...
description: >-
  ...
  file-based wallet templates, shows schema and wallet/public flags, loads
  simple example data, and creates Apple or Google wallets through POST
  /v1/wallets. Use when the user asks to browse wallet templates, inspect a
  template structure, open the wallet demo, or request a sample wallet.
disable-model-invocation: true
---

# Wallet demo browser

Open the local dashboard that lists available templates, shows each template's
schema and field flags, and lets the user request a wallet with editable JSON.

## When invoked

1. Confirm mi-wallet-service is reachable at `http://localhost:3000/health`
   unless the user set `WALLET_SERVICE_URL`. If it is not, tell the user to
   start the app and LocalStack; the template browser can still open, but
   wallet creation will fail until the service is available.
...
3. Start the dashboard:
   ...
   Point at a deployed service (for example devops) with:

   ```bash
   WALLET_SERVICE_URL=https://mi-wallet-service.devops.petscreening.com \
     bash .cursor/skills/wallet-demo-browser/scripts/start.sh
   ```
...
- Selecting a card shows `data` fields from `schema.json` (type, required)
  and `wallet` / `public` flags from `template.json`, plus the raw schema.
